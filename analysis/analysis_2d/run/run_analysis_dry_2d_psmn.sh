...
combine_output "${tmpdir1}/mppnccombine" "$run_dir" "$date_name"
...
function analysis_loop {
    ...
    # require fms_output_freq, days_per_segment, num_fourier, PotTmin, PotTmax
    # sets global variable successful_analysis
    local input_dir="${run_dir}/combine"      # directory where combined netcdf files are written
    local output_dir="${run_dir}/history"     # directory where output is written
    local run_analysis="${run_dir}/analysis"
    mkdir -p "$output_dir"
    cd "${run_analysis}/${date_name}"

    local times_per_segment=$((days_per_segment * ${fms_output_freq:0:1} )) # number of instances in each segment
	...
	if [ $num_segments -eq 1 ]; then
	    local output_file_name="${output_dir}/${date_name}.nc"
	    local TimeIn=0
	else
	    local output_file_name="${output_dir}/${date_name}.segment${isegment}.nc"
	    local TimeIn=$(( (isegment - 1) * times_per_segment ))
	    echo "Segment ${isegment}"
	    echo "TimeIn = $TimeIn"
	fi

	local filename_list=$(cat -e <<EOF
    &filename_list
...
analysis_loop "$run_dir" "$date_name" "$num_segments"
...
if [ $successful_analysis -eq 1 ]; then
    move_output "$run_dir" "$date_name" "$data_dir" "$fms_surface_freq"
...
function remove_analysis_files {
    local run_dir="$1"
    local date_name="$2"
    local input_dir="${run_dir}/combine"                     # directory where combined netcdf files are written
    local output_dir="${run_dir}/history"                     # directory where output is written
...
remove_model_output "$data_dir" "$run_dir" "date_name" "$num_segments"
